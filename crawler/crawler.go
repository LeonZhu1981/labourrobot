...
func RequestInsuranceDataByHttp(typeID string) (responseBody string, err error) {
	...
	if err == nil {
		if resp.StatusCode == 200 {
			...
			body, _ := ioutil.ReadAll(resp.Body)
			responseBody = string(body)
			return responseBody, errors.New(fmt.Sprintf("Http request failed. The response status code: %d", resp.StatusCode))
		}
	} else {
		return "", err
	}
